...
if not st.session_state.autenticato:
    ...
    with col_full:
        st.markdown("<p style='color:white; text-align:center; font-size: 1rem;'>Accesso Utenti Registrati</p>", unsafe_allow_html=True)
        codice = st.text_input("Login", type="password", label_visibility="collapsed", placeholder="Inserisci il tuo codice")
...
def mostra_timer():
    if st.session_state.start_time and st.session_state.get("simulazione", False):
        rimanente = max(0, 3600 - (time.time() - st.session_state.start_time))
