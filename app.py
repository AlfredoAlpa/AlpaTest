...
# --- LOGIN (MODIFICATO CON TESTI PIÙ GRANDI - MODIFICA 2) ---
...
if not st.session_state.autenticato:
    st.markdown("""
        ...
        .titolo-box { color: #FFD700 !important; font-size: 2.4rem !important; font-weight: 900 !important; display: block !important; margin-bottom: 10px; }
        .benvenuto-testo { color: #FFFFFF !important; font-size: 1.25rem !important; line-height: 1.5; margin-bottom: 30px !important; display: block; text-align: center; }
        .istruzione-box { color: #FFD700 !important; font-size: 1.1rem !important; font-weight: bold; display: block !important; margin-bottom: 10px !important; }
...
# --- CSS GENERALE (TESTI INGRANDITI - MODIFICA 2) ---
st.markdown("""
    <style>
...
# --- FUNZIONI (MODIFICA 1: RERUN RIMOSSO) ---
...
def importa_quesiti():
    try:
        ...
        if frames:
            ...
            # st.rerun() rimosso per evitare l'avviso giallo (Modifica 1)
                ...
                if df_online.shape[1] >= 3:
                    ...
                    if scelta != "-- Seleziona --":
                        ...
                        if st.button("📖 APRI DISPENSA", use_container_width=True):
                            ...
                            st.rerun()
            except Exception as e: st.error(f"Errore tecnico nel database: {e}")
...
# --- COLONNA DESTRA (MATERIE INGRANDITE - MODIFICA 3) ---
with col_dx:
    ...
    for i in range(9):
        ...
        with c_a: 
            # Qui abbiamo applicato la classe CSS 'nome-materia' per ingrandire il testo (Modifica 3)
...
if st.session_state.fase == "FINE":
    ...
    st.success(f"### Test Completato! \n\n Risposte esatte: {esatte} su {len(st.session_state.df_filtrato)}")
